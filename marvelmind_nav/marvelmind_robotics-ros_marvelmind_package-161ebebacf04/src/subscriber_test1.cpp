...
//Topic for the average robot position:
#include "marvelmind_nav/robot_pos"
...
#define HEDGE_POSITION_AVERAGE_TOPCIC_NAME "/robot_pos" 
...
void hedgePosAngCallback(const marvelmind_nav::robot_pos& robot_pos_msg)
{
  ROS_INFO("Hedgehog data: Address= %d, timestamp= %d, X=%.3f  Y= %.3f  Z=%.3f  Angle: %.1f  flags=%d", 	
				(int) robot_pos_msg.address, 
				(int) robot_pos_msg.timestamp_ms, 
				//(float) robot_pos_msg.x_m, (float) robot_pos_msg.y_m, (float) robot_pos_msg.z_m,
				(float) robot_pos_msg.data[0], (float) robot_pos_msg.data[1], (float) 0.0
				(float) robot_pos_msg.angle,  
				(int) robot_pos_msg.flags);
				
  if ((robot_pos_msg.flags&(1<<0))==0)
    {				
      showRvizObject(robot_pos_msg.address,robot_pos_msg.data[0], robot_pos_msg.data[1], 0.0, objHedge);
    }  
}
...
int main(int argc, char **argv)
{
  ...
  ros::Subscriber subHedgeWithAngle = rosNode.subscribe(HEDGE_POSITION_AVERAGE_TOPCIC_NAME, 1000, hedgePosAngCallback);
